...
-- =============================================================================
-- 1. ENTITY SEARCH
--    entity_id is TEXT because entities.id is UUID
-- =============================================================================
...
--    supporting_evidence cast TEXT because the column is JSONB
-- =============================================================================
...
-- =============================================================================
-- 7. GET FULL MEMORY CONTEXT
--    sub_themes / key_takeaways returned as TEXT (cast from JSONB)
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    t.sub_themes::TEXT,
    t.conversation_type,
    t.key_takeaways::TEXT,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'name', e.entity_name, 'type', e.entity_type, 'context', e.context
    ...
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        ...
        'reasoning', d.reasoning, 'confidence', d.confidence_level
      )) FROM decisions d WHERE d.memory_id = m.id),
      '[]'::jsonb
    ) AS decisions,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'reflection', r.reflection, 'type', r.reflection_type,
        'topic', r.topic, 'tone', r.emotional_tone
      )) FROM reflections r WHERE r.memory_id = m.id),
      '[]'::jsonb
    ) AS reflections,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'insight', s.insight, 'category', s.insight_category,
        'confidence', s.confidence, 'action', s.suggested_action
      )) FROM strategic_insights s WHERE s.memory_id = m.id),
      '[]'::jsonb
    ) AS strategic_insights,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'task', tk.task, 'urgency', tk.urgency,
        'category', tk.category, 'status', COALESCE(tk.status, 'open')
...
-- =============================================================================
-- 9. SEARCH BY THEME
--    sub_themes cast to TEXT for ILIKE comparison
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    (
      ...
      CASE WHEN t.sub_themes::TEXT   ILIKE '%' || theme_keywords || '%' THEN 2 ELSE 0 END +
      CASE WHEN m.title              ILIKE '%' || theme_keywords || '%' THEN 2 ELSE 0 END +
      CASE WHEN m.summary            ILIKE '%' || theme_keywords || '%' THEN 1 ELSE 0 END +
      CASE WHEN m.transcript         ILIKE '%' || theme_keywords || '%' THEN 1 ELSE 0 END
  ...
  WHERE m.status = 'completed'
    AND (
      ...
      OR t.sub_themes::TEXT ILIKE '%' || theme_keywords || '%'
      OR m.title         ILIKE '%' || theme_keywords || '%'
      OR m.summary       ILIKE '%' || theme_keywords || '%'
      OR m.transcript    ILIKE '%' || theme_keywords || '%'
    )
...
-- =============================================================================
-- 10. SEMANTIC SEARCH (embedding vector — called by n8n, not MCP directly)
-- =============================================================================
...
-- =============================================================================
-- 11. ENRICHMENT (called by n8n pipelines)
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (SELECT jsonb_agg(DISTINCT jsonb_build_object(
        'decision', d.decision, 'category', d.category, 'reasoning', d.reasoning
      )) FROM decisions d WHERE d.memory_id = m.id),
      '[]'::jsonb
    ) AS decisions,
    COALESCE(
      (SELECT jsonb_agg(DISTINCT jsonb_build_object(
        'reflection', r.reflection, 'type', r.reflection_type, 'topic', r.topic
      )) FROM reflections r WHERE r.memory_id = m.id),
      '[]'::jsonb
    ) AS reflections,
    COALESCE(
      (SELECT jsonb_agg(DISTINCT jsonb_build_object(
        'insight', s.insight, 'category', s.insight_category, 'action', s.suggested_action
      )) FROM strategic_insights s WHERE s.memory_id = m.id),
      '[]'::jsonb
    ) AS strategic_insights,
    COALESCE(
      (SELECT jsonb_agg(DISTINCT jsonb_build_object(
        'customer', c.customer_name, 'type', c.customer_type,
        'pain_point', c.pain_point, 'desire', c.desire
      )) FROM customer_insights c WHERE c.memory_id = m.id),
      '[]'::jsonb
    ) AS customer_insights,
    COALESCE(
      (SELECT jsonb_agg(DISTINCT jsonb_build_object(
        'task', tk.task, 'urgency', tk.urgency, 'status', COALESCE(tk.status, 'open')
      )) FROM tasks tk WHERE tk.memory_id = m.id),
      '[]'::jsonb
    ) AS tasks,
    COALESCE(
      (SELECT jsonb_agg(DISTINCT jsonb_build_object(
        'theme', t.main_theme, 'sub_themes', t.sub_themes, 'type', t.conversation_type
...
BEGIN
  ...
  SELECT 'entities'::TEXT, 'people'::TEXT,
    (SELECT ARRAY_AGG(entity_name ORDER BY mention_count DESC)
     FROM (SELECT entity_name, COUNT(*) AS mention_count FROM entities
           WHERE entity_type = 'person' GROUP BY entity_name ORDER BY COUNT(*) DESC LIMIT 30) sub)::TEXT[],
  ...
  SELECT 'entities'::TEXT, 'companies'::TEXT,
    (SELECT ARRAY_AGG(entity_name ORDER BY mention_count DESC)
     FROM (SELECT entity_name, COUNT(*) AS mention_count FROM entities
           WHERE entity_type = 'company' GROUP BY entity_name ORDER BY COUNT(*) DESC LIMIT 30) sub)::TEXT[],
  ...
  SELECT 'entities'::TEXT, 'projects'::TEXT,
    (SELECT ARRAY_AGG(entity_name ORDER BY mention_count DESC)
     FROM (SELECT entity_name, COUNT(*) AS mention_count FROM entities
           WHERE entity_type = 'project' GROUP BY entity_name ORDER BY COUNT(*) DESC LIMIT 30) sub)::TEXT[],
  ...
  SELECT 'entities'::TEXT, 'tools'::TEXT,
    (SELECT ARRAY_AGG(entity_name ORDER BY mention_count DESC)
     FROM (SELECT entity_name, COUNT(*) AS mention_count FROM entities
           WHERE entity_type = 'tool' GROUP BY entity_name ORDER BY COUNT(*) DESC LIMIT 30) sub)::TEXT[],
  ...
  SELECT 'reflections'::TEXT, 'topics'::TEXT,
    (SELECT ARRAY_AGG(topic ORDER BY cnt DESC)
     FROM (SELECT topic, COUNT(*) AS cnt FROM reflections WHERE topic IS NOT NULL
  ...
  SELECT 'customer_insights'::TEXT, 'customer_names'::TEXT,
    (SELECT ARRAY_AGG(customer_name ORDER BY cnt DESC)
     FROM (SELECT customer_name, COUNT(*) AS cnt FROM customer_insights
  ...
  SELECT 'themes'::TEXT, 'main_themes'::TEXT,
    (SELECT ARRAY_AGG(main_theme ORDER BY cnt DESC)
     FROM (SELECT main_theme, COUNT(*) AS cnt FROM themes WHERE main_theme IS NOT NULL
...
GRANT EXECUTE ON FUNCTION agent_get_decisions(TEXT, INT)           TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION agent_get_reflections(TEXT, INT)         TO anon, authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION agent_get_memory_context(BIGINT)         TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION agent_get_recent_memories(INT, TEXT)     TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION agent_search_by_theme(TEXT, INT)         TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION agent_search_memories_by_embedding(VECTOR(1536), FLOAT, INT) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_memory_enrichment(BIGINT[])          TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION agent_discover_database()                TO anon, authenticated, service_role;
